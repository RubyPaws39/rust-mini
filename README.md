...
Current VM coverage includes integer and boolean math, control flow, function calls, string literals, string concatenation, array literals, vec literals, indexing, `len`, `concat`, `contains`, `format!`, `print!`, `println!`, and pure string methods like `.trim()`, `.to_uppercase()`, `.replace()`, `.contains()`, `.starts_with()`, and `.ends_with()`.
